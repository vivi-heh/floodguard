...
export function generateMarkdownReport(data: ReportData): string {
  ...
  const metPts = (Number(sub.meteorologicalHazard ?? 50) * 0.34).toFixed(1);
  const topoPts = (Number(sub.topographicVulnerability ?? 50) * 0.20).toFixed(1);
  const hydroPts = (Number(sub.hydrologicalStress ?? 50) * 0.20).toFixed(1);
  const soilPts = (Number(sub.soilSaturationRunoff ?? 50) * 0.16).toFixed(1);
  const socioPts = (Number(sub.socioInfrastructureExposure ?? 50) * 0.10).toFixed(1);

  const safeLat = typeof point.latitude === 'number' ? point.latitude.toFixed(4) : '0.0000';
  const safeLng = typeof point.longitude === 'number' ? point.longitude.toFixed(4) : '0.0000';
  const safeRisk = typeof prediction?.riskScore === 'number' ? prediction.riskScore.toFixed(1) : '0.0';
  const safeConf = typeof prediction?.confidenceScore === 'number' ? prediction.confidenceScore.toFixed(1) : '90.0';

  const factors = reasoning?.contributingFactors || (reasoning as any)?.factors || [];
...
Geographic Coordinates: ${safeLat}° N, ${safeLng}° E
Mean Basin Elevation:   ${point.elevation} m MSL
Soil Pedology:          ${point.soilType}
Land Surface Cover:     ${point.landCover}
Demographic Density:    ${point.populationDensity?.toLocaleString() ?? 0} residents/km²
Drainage Infrastructure:${point.infrastructureScore ?? 5} / 10 rating
Historical Inundation:  ${point.historicalFloods ?? 0} documented flood events

--------------------------------------------------------------------------------
2. COMPOSITE AREA RISK DETECTION & CLASSIFICATION
--------------------------------------------------------------------------------
Overall Risk Score:     ${safeRisk} / 100
Assigned Hazard Rating: [${prediction?.riskLevel ?? 'LOW'}] RISK
Flash Flood / Surge:    ${prediction?.flashFloodAlert ? 'CRITICAL ALERT (Sudden Runoff Trigger)' : 'STABLE (Below Flash Threshold)'}
Model Confidence:       ${safeConf}%

MULTI-FACTORIAL AREA SCORE DECOMPOSITION (5-AXIS SYNTHESIS):
• Meteorological Hazard (34% Weight):
    Raw Index: ${Number(sub.meteorologicalHazard ?? 50).toFixed(1)}/100 | Score Contribution: +${metPts} pts
    Description: 24h precipitation rate, storm cell kinetics, and atmospheric moisture.
• Hydrological Riverine Stress (20% Weight):
    Raw Index: ${Number(sub.hydrologicalStress ?? 50).toFixed(1)}/100 | Score Contribution: +${hydroPts} pts
...
• Topographic Slope & Elevation Profile (20% Weight):
    Raw Index: ${Number(sub.topographicVulnerability ?? 50).toFixed(1)}/100 | Score Contribution: +${topoPts} pts
...
• Soil Saturation & Runoff Ratio (16% Weight):
    Raw Index: ${Number(sub.soilSaturationRunoff ?? 50).toFixed(1)}/100 | Score Contribution: +${soilPts} pts
    Description: Substrate pore-water retention and infiltration capacity threshold.
• Socio-Infrastructure Exposure (10% Weight):
    Raw Index: ${Number(sub.socioInfrastructureExposure ?? 50).toFixed(1)}/100 | Score Contribution: +${socioPts} pts
...
River Flow Discharge:   ${point.riverDischarge?.toLocaleString() ?? 0} m³/s
...
${(prediction?.featureImportance || []).map((f, i) => {
  ...
  return `[${i + 1}] ${f.name.padEnd(28)}: ${imp.toFixed(1)}% contribution`;
}).join('\n')}
...
// Reliable browser file download helper without premature URL revocation
export function triggerFileDownload(filename: string, content: string, mimeType: string): { success: boolean; url: string; filename: string } {
  try {
...
export function downloadTextReport(data: ReportData): { success: boolean; content: string; filename: string; url: string } {
  const content = generateMarkdownReport(data);
  const safeName = (data.point?.locationName || 'Location').replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
  const filename = `FloodGuard_Report_${safeName}_2026.txt`;
  const res = triggerFileDownload(filename, content, 'text/plain;charset=utf-8');
  return { ...res, content, filename };
}

export function generateJsonReportString(data: ReportData): string {
  ...
  const metRaw = Number(sub.meteorologicalHazard ?? 50);
  const hydroRaw = Number(sub.hydrologicalStress ?? 50);
  const topoRaw = Number(sub.topographicVulnerability ?? 50);
  const soilRaw = Number(sub.soilSaturationRunoff ?? 50);
  const socioRaw = Number(sub.socioInfrastructureExposure ?? 50);
  ...
  return JSON.stringify({
    ...
    catchment_geography: {
      location_name: data.point?.locationName || "Monitored Catchment",
      coordinates: {
        latitude: data.point?.latitude ?? 0,
        longitude: data.point?.longitude ?? 0,
        elevation_meters_msl: data.point?.elevation ?? 0
      },
      basin_characteristics: {
        soil_pedology: data.point?.soilType || "Alluvial",
        land_cover: data.point?.landCover || "Mixed",
        population_density_per_km2: data.point?.populationDensity ?? 0,
        drainage_infrastructure_rating_10: data.point?.infrastructureScore ?? 5,
        recorded_historical_flood_events: data.point?.historicalFloods ?? 0
      }
    },
    risk_assessment: {
      overall_risk_score: Number((data.prediction?.riskScore ?? 0).toFixed(1)),
      risk_level: data.prediction?.riskLevel || 'LOW',
      flash_flood_alert: Boolean(data.prediction?.flashFloodAlert),
      model_confidence_percent: Number((data.prediction?.confidenceScore ?? 90).toFixed(1))
    },
      ...
      weights_and_contributions: [
        {
          ...
          weight_percent: 34,
          raw_score: Number(metRaw.toFixed(1)),
          points_contributed: Number((metRaw * 0.34).toFixed(1)),
          inputs: {
            rainfall_24h_mm: data.point?.rainfall ?? 0,
            live_rain_rate_mm_h: data.point?.liveWeather?.precipitationRate ?? 0,
            humidity_percent: data.point?.humidity ?? 0
          }
        },
        {
          axis: "Hydrological Riverine Stress",
          weight_percent: 20,
          raw_score: Number(hydroRaw.toFixed(1)),
          points_contributed: Number((hydroRaw * 0.20).toFixed(1)),
          inputs: {
            river_discharge_m3_s: data.point?.riverDischarge ?? 0,
            water_level_m: data.point?.waterLevel ?? 0
          }
        },
        {
          axis: "Topographic Slope & Elevation Profile",
          weight_percent: 20,
          raw_score: Number(topoRaw.toFixed(1)),
          points_contributed: Number((topoRaw * 0.20).toFixed(1)),
          inputs: {
            elevation_m: data.point?.elevation ?? 0
          }
        },
        {
          axis: "Soil Saturation & Runoff Ratio",
          weight_percent: 16,
          raw_score: Number(soilRaw.toFixed(1)),
          points_contributed: Number((soilRaw * 0.16).toFixed(1)),
          inputs: {
            soil_type: data.point?.soilType || "Alluvial",
        ...
        },
        {
          ...
          weight_percent: 10,
          raw_score: Number(socioRaw.toFixed(1)),
          points_contributed: Number((socioRaw * 0.10).toFixed(1)),
          inputs: {
            population_density: data.point?.populationDensity ?? 0,
            infrastructure_score: data.point?.infrastructureScore ?? 5
          }
        }
      ],
    ...
    sensor_telemetry: {
      rainfall_24h_mm: data.point?.rainfall ?? 0,
      river_discharge_m3_s: data.point?.riverDischarge ?? 0,
      water_level_meters: data.point?.waterLevel ?? 0,
      temperature_celsius: data.point?.temperature ?? 0,
      relative_humidity_percent: data.point?.humidity ?? 0,
    ...
    explainable_ai_feature_attributions: (data.prediction?.featureImportance || []).map(f => {
      ...
      return {
        feature: f.name,
        contribution_percent: Number(imp.toFixed(1))
      };
    }),
    mitigation_and_reasoning: {
      executive_summary: data.reasoning?.summary || "Automated multi-factor geospatial assessment completed.",
      vulnerability_factors: data.reasoning?.contributingFactors || (data.reasoning as any)?.factors || [],
      action_recommendations: data.reasoning?.recommendations || []
    }
  }, null, 2);
}

export function downloadJsonReport(data: ReportData): { success: boolean; content: string; filename: string; url: string } {
  const jsonContent = generateJsonReportString(data);
  const safeName = (data.point?.locationName || 'Location').replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
  const filename = `FloodGuard_Telemetry_${safeName}_2026.json`;
  const res = triggerFileDownload(filename, jsonContent, 'application/json;charset=utf-8');
  return { ...res, content: jsonContent, filename };
}

export function openReportInNewWindow(data: ReportData): boolean {
  try {
    ...
    if (!win) {
      // In case popup blocker intercepts, fallback to downloading html
      ...
      <body>
            ...
            <tr>
              ...
              <td style="width: 50%;">
                <div class="bar-container">
                  <div class="bar-fill" style="width: ${Math.min(imp, 100)}%;"></div>
                </div>
              </td>
            </tr>
            `;
        ...
        <div class="two-col">
          <div class="rec-box" style="margin-bottom: 0;">
            ...
            ${(reasoning?.contributingFactors || (reasoning as any)?.factors || []).map(f => `<div class="rec-item">${f}</div>`).join('')}
          </div>
          <div class="rec-box" style="margin-bottom: 0;">
...
export function downloadHtmlReport(data: ReportData): { success: boolean; content: string; filename: string; url: string } {
  const html = generateHtmlReport(data);
  const safeName = (data.point?.locationName || 'Location').replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
  const filename = `FloodGuard_Executive_Report_${safeName}_2026.html`;
  const res = triggerFileDownload(filename, html, 'text/html;charset=utf-8');
  return { ...res, content: html, filename };
}

export function printExecutiveReport(data: ReportData) {
  // Trigger the HTML download first
  const res = downloadHtmlReport(data);

  // In addition, attempt opening the print dialog via an invisible iframe without trigger popup blockers
  try {
    ...
    if (doc) {
      doc.open();
      doc.write(generateHtmlReport(data));
      doc.close();
      ...
      setTimeout(() => {
        try {
          ...
          // Ignore if iframe print is not permitted in sandbox
        }
        setTimeout(() => {
          try {
    ...
    // Fallback handled by downloadHtmlReport
  }
  return res;
}
